...
std::vector<std::pair<int, int>> detect_silence(const AudioSegment& audio_segment, int min_silence_len, double silence_thresh, int seek_step) {
    int seg_len = audio_segment.length(); // Get length of the audio segment in milliseconds
    ...
    for (int i = 0; i <= last_slice_start; i += seek_step) {
        ...
        AudioSegment audio_slice = audio_segment.slice(i, i + min_silence_len);
    ...
    if (last_slice_start % seek_step != 0) {
        AudioSegment audio_slice = audio_segment.slice(last_slice_start, last_slice_start + min_silence_len);
        if (audio_slice.rms() <= silence_threshold) {
...
std::vector<std::pair<int, int>> detect_nonsilent(const AudioSegment& audio_segment, int min_silence_len, double silence_thresh, int seek_step) {
...
std::vector<AudioSegment> split_on_silence(const AudioSegment& audio_segment, int min_silence_len, double silence_thresh, int keep_silence, int seek_step) {
    if (keep_silence < 0) {
    ...
    for (const auto& range : output_ranges) {
        ...
        int end = std::min(range.second, static_cast<int>(audio_segment.length()));
        if (start < end) {
            segments.push_back(audio_segment.slice(start, end));
        }
    }
...
int detect_leading_silence(const AudioSegment& sound, double silence_threshold, int chunk_size) {
    if (chunk_size <= 0) {
    ...
    while (trim_ms + chunk_size <= sound.length()) {
        // Slice the audio segment and calculate the dBFS value
        AudioSegment chunk = sound.slice(trim_ms, trim_ms + chunk_size);
        double chunk_dBFS = chunk.dBFS(); // Assume dBFS method is available in AudioSegment
    ...
    return std::min(trim_ms, static_cast<int>(sound.length()));
}